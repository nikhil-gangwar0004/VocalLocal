...
import { User, LogOut, Menu, X, ChevronDown, Briefcase } from 'lucide-react';
            ...
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: 999, padding: '6px 12px 6px 6px', cursor: 'pointer', color: '#fff',
                }}
              >
                <div style={{
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {!isMobile && (
                <Link to="/login" style={{ textDecoration: 'none' }}>
                  <button style={{ padding: '8px 18px', borderRadius: 999, background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                    Login
                  </button>
                </Link>
              )}
              <Link to="/signup" style={{ textDecoration: 'none' }}>
                <button style={{ padding: '8px 18px', borderRadius: 999, background: '#fbbf24', border: 'none', color: '#000', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                  Sign Up
                </button>
              </Link>
            </div>
        ...
        {menuOpen && isMobile && (
          <motion.div
            ...
            style={{
              ...
              background: '#0f172a', borderBottom: '1px solid rgba(251,191,36,0.2)',
              paddingBottom: 16,
            }}
          >
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  color: location.pathname === link.path ? '#fbbf24' : '#e2e8f0',
                  borderLeft: location.pathname === link.path ? '3px solid #fbbf24' : '3px solid transparent',
                }}>
                  {link.label}
                </div>
              </Link>
            ...
            {user && user.role === 'customer' && (
              <Link to="/become-expert" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '14px 24px', color: '#fbbf24', fontWeight: 700, fontSize: 15 }}>
                  Become an Expert 🛠️
                </div>
              </Link>
            )}
            ...
            {user && (user.role === 'professional' || user.email === 'gangwarn411@gmail.com') && (
              <Link to={user.email === 'gangwarn411@gmail.com' ? '/admin' : '/expert-panel'} onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '14px 24px', color: '#4ade80', fontWeight: 700, fontSize: 15 }}>
                  Dashboard ⚡
                </div>
              </Link>
            )}
            ...
            {!user && (
              <Link to="/login" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none' }}>
                <div style={{ padding: '14px 24px', color: '#94a3b8', fontWeight: 600, fontSize: 15 }}>
                  Login
                </div>
              </Link>
            )}
